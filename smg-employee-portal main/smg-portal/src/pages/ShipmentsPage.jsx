...
export default function ShipmentsPage({ role = "admin" }) {
  ...
  const columns = [
    {
      ...
      render: (v, row) => (
        <button
          ...
          className="font-mono font-bold text-brand text-xs hover:underline"
        >
          {v}
        </button>
      ),
    ...
    ...(role === "admin"
      ? [{ key: "partner", label: "Partner", width: "110px" }]
      : []),
      ...
      ? [
          {
            ...
            render: (_, row) => (
              <div className="flex items-center gap-1">
                <button
                  onClick={() => navigate(`/track/${row.trackingNo}`)}
  ...
  return (
    <DashboardLayout
      pageTitle={role === "partner" ? "My Deliveries" : "Shipments"}
    >
      {/* Header row */}
      ...
      <Modal
        ...
        title={`Shipment — ${selected?.trackingNo}`}
        size="lg"
      >
        {selected && (
          <div className="space-y-4 text-sm">
            ...
            <div className="flex gap-2 pt-2 border-t border-slate-100">
              <button
                onClick={() => {
                  setDetailOpen(false);
                  navigate(`/track/${selected.trackingNo}`);
                }}
                className="btn-primary text-xs"
              >
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        senderAddress: form.origin,
        receiverAddress: form.destination,
