...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await login({ email: form.userId, password: form.password, role: form.role })
        ...
        <div className="flex-1 flex flex-col justify-center px-8 py-8 overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-2">
              {ROLES.map(({ id, label, icon: Icon, desc }) => (
                <button
                  key={id}
                  type="button"
                  onClick={() => { setForm(p => ({ ...p, role: id })); setError('') }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-sm text-slate-500">
              ...
              <Link to="/register" className="text-[#1a6ab1] font-semibold hover:underline">
                Create Account
              </Link>
            </p>
          </form>
